...
/*
 * Transforms the gray information of an image stored in buffer to it's rgb
 * representation
 */

int GrayToRgb(byte *gray, byte **rgb, int buffer_size) {
    // Take size for rgb image and allocate memory
    int rgb_size = buffer_size * 3;

    *rgb = malloc(sizeof(byte) * rgb_size);
